...
class ReadHtmlCacheBehavior extends Behavior {
    ...
    static private function requireHtmlCache() {
        // 分析当前的静态规则
         ...
         if(!empty($htmls)) {
            ...
            // 检测静态规则
            $controlerName = strtolower(CONTROLER_NAME);
            $actionName = strtolower(ACTION_NAME);
            if(isset($htmls[$controlerName.':'.$actionName])) {
                $html   =   $htmls[$controlerName.':'.$actionName];   // 某个控制器的操作的静态规则
            }elseif(isset($htmls[$controlerName.':'])){// 某个控制器的静态规则
                $html   =   $htmls[$controlerName.':'];
            }elseif(isset($htmls[$actionName])){
                $html   =   $htmls[$actionName]; // 所有操作的静态规则
            }elseif(isset($htmls['*'])){
                $html   =   $htmls['*']; // 全局静态规则
            }elseif(isset($htmls['empty:index']) && !class_exists(CONTROLER_NAME.'Action')){
                $html   =    $htmls['empty:index']; // 空控制器静态规则
            }elseif(isset($htmls[$controlerName.':_empty']) && $this->isEmptyAction(CONTROLER_NAME,ACTION_NAME)){
                $html   =    $htmls[$controlerName.':_empty']; // 空操作静态规则
            }
            if(!empty($html)) {
                // 解读静态规则
                $rule   = $html[0];
                ...
                // 特殊系统变量
                $rule   = str_ireplace(
                    array('{:app}','{:controler}','{:action}','{:module}'),
                    array(APP_NAME,CONTROLER_NAME,ACTION_NAME,defined('MODULE_NAME')?MODULE_NAME:''),
                    $rule);
                // {|FUN} 单独使用函数
    ...
    //检测是否是空操作
    static private function isEmptyAction($controler,$action) {
        $className  =   $controler.'Action';
        $class      =   new $className;
