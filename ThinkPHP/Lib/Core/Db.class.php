...
class Db {
    ...
    // 查询表达式
    protected $selectSql  = 'SELECT%DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%MODULE%%HAVING%%ORDER%%LIMIT% %UNION%';
    ...
    protected function parseModule($module) {
        return !empty($module)? ' MODULE BY '.$module:'';
    }
    ...
    public function parseSql($sql,$options=array()){
        $sql   = str_replace(
            array('%TABLE%','%DISTINCT%','%FIELD%','%JOIN%','%WHERE%','%MODULE%','%HAVING%','%ORDER%','%LIMIT%','%UNION%'),
            array(
                $this->parseTable($options['table']),
                ...
                $this->parseWhere(isset($options['where'])?$options['where']:''),
                $this->parseModule(isset($options['module'])?$options['module']:''),
