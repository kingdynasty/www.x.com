 ...
 * 3，一个用户可以属于多个用户组(think_auth_module_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(think_auth_module 定义了用户组权限)
 * 
 * 4，支持规则表达式。
...
-- ----------------------------
-- think_auth_module_access 用户组明细表
-- uid:用户id，module_id：用户组id
-- ----------------------------
DROP TABLE IF EXISTS `think_auth_module_access`;
CREATE TABLE `think_auth_module_access` (
  `uid` mediumint(8) unsigned NOT NULL,
  `module_id` mediumint(8) unsigned NOT NULL,
  UNIQUE KEY `uid_2` (`uid`,`module_id`),
  KEY `uid` (`uid`),
...
class Authority {
    ...
    protected $_config = array(
        ...
        'AUTH_MODULE' => 'think_auth_module', //用户组数据表名
        'AUTH_MODULE_ACCESS' => 'think_auth_module_access', //用户组明细表
    ...
    public function getModules($uid) {
        ...
        if (!empty($modules[$uid]))
            return $modules[$uid];
        $modules[$uid] = M()->table($this->_config['AUTH_MODULE_ACCESS'] . ' a')->where("a.uid='$uid'")->join($this->_config['AUTH_MODULE']." g on a.module_id=g.id")->select();
        return $modules[$uid];
    }
