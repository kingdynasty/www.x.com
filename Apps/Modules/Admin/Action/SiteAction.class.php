...
class SiteAction extends BaseAction {
	...
	public function init() {
		...
		$list = $this->db->field('*')->page($page,20)->select();
		$pages = $this->db->pages;
		$show_dialog = true;
		...
		include Admin::adminTpl('site_list');
	}
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->data(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style))->add()) {
				$class_site = import('Sites');
				$class_site->setCache();
			...
			$release_point_list = M('ReleasePoint')->field('id, name')->select();
			$show_validator = $show_scroll = $show_header = true;
			$template_list = template_list();
			include Admin::adminTpl('site_add');
		}
	}
	...
	public function del() {
		...
		if ($this->db->where(array('siteid'=>$siteid))->find()) {
	...
	public function edit() {
		$siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
		if ($data = $this->db->where(array('siteid'=>$siteid))->find()) {
			if (isset($_POST['dosubmit'])) {
				$name = isset($_POST['name']) && trim($_POST['name']) ? trim($_POST['name']) : showmessage(L('site_name').L('empty'));
				...
				$template = isset($_POST['template']) && !empty($_POST['template']) ? $_POST['template'] : showmessage(L('please_select_a_style'));
				$default_style = isset($_POST['default_style']) && !empty($_POST['default_style']) ? $_POST['default_style'] : showmessage(L('please_choose_the_default_style'));	
				if ($data['name'] != $name && $this->db->where(array('name'=>$name))->field('siteid')->find()) {
					showmessage(L('site_name').L('exists'));
				}
				if ($siteid != 1) {
					if (!preg_match('/^\\w+$/i', $dirname)) {
					...
					if ($data['dirname'] != $dirname && $this->db->where(array('dirname'=>$dirname))->field('siteid')->find()) {
						showmessage(L('site_dirname').L('exists'));
					}
				} 
				...
				if (!empty($domain) && $data['domain'] != $domain && $this->db->where(array('domain'=>$domain))->field('siteid')->find()) {
					showmessage(L('site_domain').L('exists'));
				}
				if (!empty($release_point) && is_array($release_point)) {
					if (count($release_point) > 4) {
				...
				if ($this->db->where(array('siteid'=>$siteid))->data($sql)->save()) {
					$class_site = import('Sites');
					$class_site->setCache();
				...
				$release_point_db = M('ReleasePoint');
				$release_point_list = $release_point_db->pcSelect('', 'id, name');
				include Admin::adminTpl('site_edit');
			}
		} else {
			showmessage(L('notfound'), HTTP_REFERER);
		}
	...
	public function publicName() {
		...
		if ($siteid) {
			...
			$data = $this->db->where(array('siteid'=>$siteid))->field('name')->find();
	...
	public function publicDirname() {
		$dirname = isset($_GET['dirname']) && trim($_GET['dirname']) ? (C('charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['dirname'])) : trim($_GET['dirname'])) : exit('0');
		$siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : '';
		$data = array();
		if ($siteid) {
			$data = $this->db->where(array('siteid'=>$siteid))->field('dirname')->find();
