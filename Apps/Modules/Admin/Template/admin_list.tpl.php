...
<a href="javascript:confirmurl('?m=Admin&c=AdminManage&a=delete&userid=<?php echo $info['userid']?>', '<?php echo L('admin_del_cofirm')?>')"><?php echo L('delete')?></a>
<?php } else {?>
...
<!--
	function edit(id, name) {
		window.top.art.dialog({title:'<?php echo L('edit')?>--'+name, id:'edit', iframe:'?m=Admin&c=AdminManage&a=edit&userid='+id ,width:'500px',height:'400px'}, 	function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;
		var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
	}

function card(id) {
	window.top.art.dialog({title:'<?php echo L('the_password_card')?>', id:'edit', iframe:'?m=Admin&c=AdminManage&a=card&userid='+id ,width:'500px',height:'400px'}, 	'', function(){window.top.art.dialog({id:'edit'}).close()});
}
//-->
</script>