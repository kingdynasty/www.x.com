...
<!--
	$(function(){
		...
		$("#name").formValidator({onshow:"<?php echo L('input').L('release_point_name')?>",onfocus:"<?php echo L('input').L('release_point_name')?>"}).inputValidator({min:1,onerror:"<?php echo L('input').L('release_point_name')?>"}).ajaxValidator({type : "get",url : "",data :"m=Admin&c=ReleasePoint&a=publicName&id=<?php echo $id?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('release_point_name').L('exists')?>",onwait : "<?php echo L('connecting')?>"}).defaultPassed();
...
<div class="pad-10">
<form action="?m=Admin&c=ReleasePoint&a=edit&id=<?php echo $id?>" method="post" id="myform">
<fieldset>
